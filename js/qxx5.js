...
/* ==========================================================================
   壁纸变量配置使用说明：
   1. 叠加半透明颜色遮罩：+mask:rgba(127,225,221,0.2) 或 +mask:rgba(0,0,0,0.3)
   2. 混合模式（可选）：+blend:soft-light 或 +blend:overlay / multiply
   3. 平铺与尺寸：#repeat 或 #repeat:32px 或 #repeat:64px
   4. 滤镜调节：+sat:0.6（饱和度） / +contrast:0.8（对比度） / +bright:1.2（明暗）
   ========================================================================== */
const BG_TOP_LEFT = 'https://textures.neocities.org/textures/abstract-brown-and-grey/397.GIF#repeat+mask:rgba(127,225,221,0.2)';
const BG_TOP_RIGHT = 'https://artwork.neocities.org/bgs/movingstars.gif#repeat';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-bg-2').addEventListener('click', () => {
        currentBgIndex = currentBgIndex === 3 ? 4 : 3;
...
function crossfadeBackground(bgVal, isLight) {
    ...
    // 1. 解析遮罩颜色 (+mask:rgba(...) 或 +mask:#hex)
    let maskColor = null;
    const maskMatch = bgVal.match(/\+mask:([^\s+@#]+)/i);
    if (maskMatch) {
        maskColor = maskMatch[1];
    }

    // 2. 解析混合模式 (+blend:soft-light / overlay / multiply 等)
    let blendMode = null;
    const blendMatch = bgVal.match(/\+blend:([a-z-]+)/i);
    if (blendMatch) {
        blendMode = blendMatch[1];
    }

    // 3. 构建 CSS 滤镜 (sat/gray/contrast/bright)
    const filters = [];
    const satMatch = bgVal.match(/\+sat:([0-9.]+)/i);
    if (satMatch) filters.push(`saturate(${satMatch[1]})`);
    
    const contrastMatch = bgVal.match(/\+contrast:([0-9.]+)/i);
    if (contrastMatch) filters.push(`contrast(${contrastMatch[1]})`);

    const grayMatch = bgVal.match(/\+gray:([0-9.]+)/i);
    if (grayMatch) filters.push(`grayscale(${grayMatch[1]})`);

    const brightMatch = bgVal.match(/\+bright:([0-9.]+)/i);
    if (brightMatch) filters.push(`brightness(${brightMatch[1]})`);

    // 4. 解析全局透明度 (@0.5)
    let targetOpacity = '1';
    const opacityMatch = bgVal.match(/@([0-9.]+)/);
    if (opacityMatch) {
        targetOpacity = opacityMatch[1];
    }

    // 5. 解析平铺与尺寸 (#repeat 或 #repeat:64px)
    ...
    cleanUrl = cleanUrl
        .replace(/#repeat(?::[^\s@+#'"]+)?/ig, '')
        .replace(/\+mask:[^\s+@#]+/ig, '')
        .replace(/\+blend:[a-z-]+/ig, '')
        .replace(/\+(sat|contrast|gray|bright):[0-9.]+/ig, '')
        .replace(/@[0-9.]+/g, '')
        .trim();
    ...
    const start = (el = null) => {
        ...
        // 应用混合模式（若指定）
        if (blendMode) {
            layer.style.backgroundBlendMode = blendMode;
        }
        ...
        if (isWebGL) {
            ...
            if (/^https?|images\/|\.\//.test(cleanUrl)) {
                // 如果设置了遮罩层，构造双层背景：linear-gradient 遮罩在上，图片在下
                const maskGradient = maskColor ? `linear-gradient(${maskColor}, ${maskColor}), ` : '';
                
                if (isRepeat) {
                    layer.style.background = `${maskGradient}url('${cleanUrl}') repeat top left`;
                    layer.style.backgroundSize = maskColor ? `auto, ${repeatSize}` : repeatSize;
                } else {
                    layer.style.background = `${maskGradient}url('${cleanUrl}') center/cover no-repeat`;
                }
            } else {
