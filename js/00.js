    ...
    tailwind.config = {
      darkMode: 'class',
      important: '#client-list-app-root', // 确保 Tailwind 所有样式仅局限于本容器，不外溢
      corePlugins: {
        preflight: false, // 禁用全局重置，防止破坏 Blogger 博客原有的样式和导航栏
      }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // XSS 字符防御性过滤函数（通过安全地配合 .innerHTML 解析 Blogger 的 &#183; 中点实体）
      ...
      viewDownloadButtons.forEach(btn => {
        btn.addEventListener('click', async (e) => {
          ...
          // 使用 innerHTML 解析可能会在 Blogger 中被实体转义的中点
      ...
      function renderSelectedRelease() {
        ...
        // 使用 innerHTML 解析可能会在 Blogger 中被实体转义的中点，保证显示正常
