...
/* ==========================================================================
   壁纸变量配置（支持大图/平铺、+color底色、+mask遮罩、+bright明暗、+sat饱和度等）：
   - 左上角（日间模式）：BG_TOP_LEFT_1 / BG_TOP_LEFT_2
   - 右上角（夜间模式）：BG_TOP_RIGHT_1 / BG_TOP_RIGHT_2
   - 左下角：BG_BOTTOM_LEFT
   - 右下角：BG_BOTTOM_RIGHT_1 / BG_BOTTOM_RIGHT_2
   ========================================================================== */
const BG_TOP_LEFT_1 = 'https://file.garden/ZWlUCY4S7Xz2vypS/archived%20backgrounds/colours/green/grn029.jpg#repeat+mask:rgba(127,225,221,0.2)';
const BG_TOP_LEFT_2 = 'https://textures.neocities.org/textures/stone-and-brick/GRYCON7.JPG#repeat+mask:rgba(127,225,221,0.2)';
...
const BG_BOTTOM_RIGHT_1 = 'https://textures.neocities.org/textures/fabric/cit60079.jpg#repeat+mask:rgba(127,225,221,0.2)';

const BG_BOTTOM_RIGHT_2 = 'https://file.garden/ZWlUCY4S7Xz2vypS/archived%20backgrounds/colours/green/dddf143.jpg#repeat+mask:rgba(127,225,221,0.2)';
...
// 将初始记录值设为 -1，确保第一次点击任何按钮都先触发第 1 张壁纸
let currentTopLeftIndex = -1;
let currentTopRightIndex = -1;
let currentBottomRightIndex = -1;

const glowingCard = document.querySelector('.glowing-card'), audio = document.getElementById('audioPlayer');
const bgPresets = [
    { name: 'top-left-1', value: BG_TOP_LEFT_1, isLight: true },        // 0
    { name: 'top-left-2', value: BG_TOP_LEFT_2, isLight: true },        // 1
    { name: 'top-right-1', value: BG_TOP_RIGHT_1, isLight: false },     // 2
    { name: 'top-right-2', value: BG_TOP_RIGHT_2, isLight: false },     // 3
    { name: 'bottom-left', value: BG_BOTTOM_LEFT, isLight: true },       // 4
    { name: 'bottom-right-1', value: BG_BOTTOM_RIGHT_1, isLight: true }, // 5
    { name: 'bottom-right-2', value: BG_BOTTOM_RIGHT_2, isLight: true }  // 6
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 左上角（日间模式）：点击按 1 ➔ 2 ➔ 1 ➔ 2 顺序循环
    document.getElementById('btn-theme-light').addEventListener('click', () => {
        currentTopLeftIndex = (currentTopLeftIndex === 0) ? 1 : 0;
        currentBgIndex = currentTopLeftIndex;
        const p = bgPresets[currentTopLeftIndex];
        switchBackground(p.value, p.isLight);
    });

    // 右上角（夜间模式）：点击按 1 ➔ 2 ➔ 1 ➔ 2 顺序循环
    document.getElementById('btn-theme-dark').addEventListener('click', () => {
        currentTopRightIndex = (currentTopRightIndex === 2) ? 3 : 2;
        currentBgIndex = currentTopRightIndex;
        const p = bgPresets[currentTopRightIndex];
        switchBackground(p.value, p.isLight);
    });
    ...
    // 右下角：点击按 1 ➔ 2 ➔ 1 ➔ 2 顺序循环
    document.getElementById('btn-bg-2').addEventListener('click', () => {
        currentBottomRightIndex = (currentBottomRightIndex === 5) ? 6 : 5;
        currentBgIndex = currentBottomRightIndex;
        const p = bgPresets[currentBottomRightIndex];
        switchBackground(p.value, p.isLight);
    });
