    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const updateThemeIcon = () => {
        ...
        if (themeIcon) {
          themeIcon.className = isDark ? 'iconify i-lucide-moon size-5' : 'iconify i-lucide-sun size-5';
        }
      };
      ...
      viewDownloadButtons.forEach(btn => {
        btn.addEventListener('click', async (e) => {
          ...
          modalReleasesSubtitle.textContent = `${repoPath} · 获取中...`;
          ...
          try {
            ...
            modalReleasesSubtitle.textContent = `${repoPath} · ${fetchedReleases.length} 个版本`;
      ...
      function renderSelectedRelease() {
        ...
        if (!rel) return;
        modalReleaseText.textContent = `${rel.tag_name} · ${rel.name || rel.tag_name}`;
      ...
      function renderAssetsTable() {
        ...
        const filteredAssets = rel.assets.filter(asset => {
          ...
          if (platform === 'macos') searchBlob += " mac os osx dmg pkg darwin apple m1 m2 m3 silicon intel";
          ...
          tr.innerHTML = `
            <td class="pl-4 pr-16 py-5 text-sm text-highlighted font-semibold max-w-[360px] sm:max-w-[440px] truncate whitespace-nowrap">
              <div class="flex flex-col">
                ...
                <div class="flex flex-wrap gap-1 mt-1">${extBadges}</div>
              </div>
            </td>
      ...
      function detectPlatform(filename) {
        ...
        if (lower.endsWith('.exe') || (lower.includes('win') && !lower.includes('darwin')) || lower.includes('setup')) return 'Windows';
