...
const BG_TOP_LEFT='images/A1.webp',BG_TOP_RIGHT='#0e0e0e',BG_BOTTOM_LEFT='webgl-shader',BG_BOTTOM_RIGHT_1='linear-gradient(rgba(0,0,0,0.15), rgba(0,0,0,0.15)), linear-gradient(15.3deg, rgba(111, 71, 133, 1) 5.6%, rgba(232, 129, 166, 1) 19.6%, rgba(237, 237, 183, 1) 42.1%, rgba(244, 166, 215, 1) 63.7%, rgba(154, 219, 232, 1) 78.7%, rgba(238, 226, 159, 1) 96.8%)',BG_BOTTOM_RIGHT_2='images/D2.jpg';
window.isParticlesEnabled=!document.body.classList.contains('light');window.userToggledParticles=false;
...
const bgPresets=[{name:'top-left',value:BG_TOP_LEFT,isLight:true},{name:'top-right',value:BG_TOP_RIGHT,isLight:false},{name:'bottom-left',value:BG_BOTTOM_LEFT,isLight:true},{name:'bottom-right-1',value:BG_BOTTOM_RIGHT_1,isLight:true},{name:'bottom-right-2',value:BG_BOTTOM_RIGHT_2,isLight:true}];
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.getElementById('btn-bg-2').addEventListener('click',()=>{currentBgIndex=currentBgIndex===3?4:3;const p=bgPresets[currentBgIndex];switchBackground(p.value,p.isLight);});
    document.querySelectorAll('.theme-toggle, .day-toggle').forEach(b=>{
        ...
        b.addEventListener('mouseenter',r);b.addEventListener('mouseleave',r);
        b.addEventListener('click',function(){r();this.classList.add('icon-scale-active');setTimeout(()=>{this.classList.remove('icon-scale-active');this.classList.add('icon-shrink-force');},350);});
    });
...
function updateParticlesDisplay(){
    const c=document.getElementById('shuicheCanvas');if(!c)return;
    if(window.isParticlesEnabled){c.style.display='block';if(window.bgEngine?.start)window.bgEngine.start();}
    else{c.style.display='none';if(window.bgEngine?.stop)window.bgEngine.stop();}
}

let bgLayerIndex=0;
function crossfadeBackground(bgVal,isLight){
    const curId=++bgChangeSequence,container=document.getElementById('bg-container');if(!container)return;
    let cleanUrl=bgVal;const match=bgVal.match(/url\(['"]?(.*?)['"]?\)/);if(match)cleanUrl=match[1];
    const isVideo=/\.(mp4|webm|ogg|mov)(\?.*)?$/i.test(cleanUrl),isWebGL=bgVal==='webgl-shader';
    const start=(el=null)=>{
        if(curId!==bgChangeSequence)return;bgLayerIndex++;const layer=document.createElement('div');
        layer.className='bg-layer';layer.style.zIndex=bgLayerIndex;
        if(isWebGL){if(typeof window.initWebGLShaderBg==='function')layer._cleanup=window.initWebGLShaderBg(layer);}
        else if(isVideo&&el){el.style.width=el.style.height="100%";el.style.objectFit="cover";layer.appendChild(el);}
        else{if(/^https?|images\/|\.\//.test(cleanUrl))layer.style.background=`url('${cleanUrl}') center/cover no-repeat`;else layer.style.background=bgVal;}
        container.appendChild(layer);document.body.classList.toggle('light',isLight);document.documentElement.style.setProperty('--fg',isLight?'black':'#f0f0f0');
        if(!window.userToggledParticles)window.isParticlesEnabled=!isLight;
        updateParticlesDisplay();void layer.offsetWidth;
        requestAnimationFrame(()=>requestAnimationFrame(()=>layer.style.opacity='1'));
        setTimeout(()=>{
            if(curId!==bgChangeSequence)return;
            container.querySelectorAll('.bg-layer').forEach(l=>{if(l!==layer){if(typeof l._cleanup==='function')l._cleanup();l.remove();}});
            updateParticlesDisplay();
        },850);
    };
    if(isWebGL)start();
    else if(isVideo){
        const v=document.createElement('video');v.src=cleanUrl;v.muted=v.loop=v.playsInline=v.autoplay=true;
        v.onloadeddata=()=>start(v);v.onerror=()=>start();
    }else if(/^https?|images\//.test(cleanUrl)){const img=new Image();img.src=cleanUrl;img.onload=img.onerror=()=>start();}
    else start();
}

function switchBackground(v,isLight=true){
    if(v==='webgl-shader'||v.includes('gradient')||/^(#|rgb|hsl)/.test(v.trim()))crossfadeBackground(v,isLight);
    else crossfadeBackground(`url('${v}')`,isLight);
}

(function initDoubleClickToggle(){
    let lastToggle=0;
    const toggle=e=>{
        let now=Date.now();if(now-lastToggle<500)return;lastToggle=now;
    ...
    document.addEventListener('dblclick',toggle);let lastTouch=0,multi=false;
    document.addEventListener('touchstart',e=>{if(e.touches.length>1)multi=true;},{passive:true});
    document.addEventListener('touchend',e=>{
        if(e.touches.length>0)return;if(multi){multi=false;lastTouch=0;return;}
        let now=Date.now();if(now-lastTouch<=400){toggle(e);lastTouch=0;}else lastTouch=now;
    });
})();

(function initBackgroundEngine(){
    const count=1000;let scene,camera,renderer,animId=null,mouseX=0,mouseY=0,halfX=window.innerWidth/2,halfY=window.innerHeight/2,clock,geom,posAttr,posArr,velArr,resizeTimeout;
    function init(){
        if(typeof THREE==='undefined'){setTimeout(init,50);return;}
        clock=new THREE.Clock();geom=new THREE.BufferGeometry();posAttr=new THREE.BufferAttribute(new Float32Array(6*count),3);
        posAttr.setUsage(THREE.DynamicDrawUsage);geom.setAttribute('position',posAttr);geom.setAttribute('velocity',new THREE.BufferAttribute(new Float32Array(2*count),1));
        posArr=posAttr.array;velArr=geom.getAttribute('velocity').array;scene=new THREE.Scene();
        ...
        renderer.setSize(window.innerWidth,window.innerHeight,false);renderer.setClearColor(0,0);
        for(let i=0;i<count;i++){
            const x=Math.random()*800-400,y=Math.random()*800-400,z=Math.random()*400-200;
            posArr[6*i]=posArr[6*i+3]=x;posArr[6*i+1]=posArr[6*i+4]=y;posArr[6*i+2]=posArr[6*i+5]=z;
        }
    ...
    function debounceResize(){
        ...
        resizeTimeout=setTimeout(()=>{camera.aspect=window.innerWidth/window.innerHeight;camera.updateProjectionMatrix();renderer.setSize(window.innerWidth,window.innerHeight);halfX=window.innerWidth/2;halfY=window.innerHeight/2;},100);
    }
    function anime(){
        let delta=Math.min(clock.getDelta(),0.1),ts=delta/(1/60);
        for(let i=0;i<count;i++){
            velArr[2*i]+=0.015*ts;velArr[2*i+1]+=0.015*ts;
            posArr[6*i+2]+=(velArr[2*i]+0.03)*ts;posArr[6*i+5]+=velArr[2*i+1]*ts;
            if(posArr[6*i+2]>200){
                let z=Math.random()*200-200;posArr[6*i+2]=posArr[6*i+5]=z;velArr[2*i]=velArr[2*i+1]=0;
            }
        }
        posAttr.needsUpdate=true;camera.position.x+=(-mouseX*0.1-camera.position.x)*0.02;camera.position.y+=(-mouseY*0.1-camera.position.y)*0.02;
        camera.lookAt(scene.position);renderer.render(scene,camera);animId=requestAnimationFrame(anime);
    }
    ...
    window.bgRenderer={setClearColor:(c,a)=>renderer?.setClearColor(c,a)};window.addEventListener('DOMContentLoaded',init);
})();
...
function togglePlay(btn){
    if(audio.paused){audio.play();btn.classList.add('playing');btn.querySelector('.btn-text').innerText='Pause';isPlaying=true;}
    else{audio.pause();btn.classList.remove('playing');btn.querySelector('.btn-text').innerText='Play';isPlaying=false;}
}
...
document.addEventListener('play',e=>{
    document.querySelectorAll('audio').forEach(el=>{
        if(el!==e.target&&!el.paused){
            el.pause();
            if(el.id==='audioPlayer'){const btn=document.querySelector('.play-btn');if(btn){btn.classList.remove('playing');btn.querySelector('.btn-text').innerText='Play';}isPlaying=false;}
        }
    });
},true);
...
function loadMusicPlayer(){
    ...
    function savePlaybackState(){if(selectedSong)localStorage.setItem('musicPlayerState',JSON.stringify({currentSongIndex:indexSong,isPlaying:!selectedSong.paused,volume:selectedSong.volume,currentTime:pendingCurrentTime>0?pendingCurrentTime:selectedSong.currentTime}));}
    ...
    function updateUIForSong(idx){
        updateInfo(songName_elmnt,songs[idx].songName);updateInfo(singerName_elmnt,songs[idx].artist);setProperty(sliderImgs_elmnt,"--index",-idx);
    ...
    function restorePlaybackState(){
        try{
            const state=localStorage.getItem('musicPlayerState');if(!state){updateUIForSong(0);return;}
            const s=JSON.parse(state);
            if(s.currentSongIndex!==undefined&&s.currentSongIndex>=0&&s.currentSongIndex<=songsLength){
                indexSong=s.currentSongIndex;if(s.currentTime!==undefined)pendingCurrentTime=s.currentTime;
                if(s.volume!==undefined)mainAudio.volume=s.volume;updateUIForSong(indexSong);
    ...
    function handleChangeMusic({isPrev=false,playListIndex=null}){
        if(isLocked)return;let newIdx=playListIndex!==null?playListIndex:(isPrev?indexSong-1:indexSong+1);
        if(newIdx<0)newIdx=songsLength;if(newIdx>songsLength)newIdx=0;if(newIdx===indexSong&&playListIndex===null)return;
        let wasPlaying=songIsPlayed;isSwitchingMusic=true;indexSong=newIdx;pendingCurrentTime=0;updateUIForSong(indexSong);mainAudio.src=songs[indexSong].files.song;
        if(wasPlaying){
            let p=mainAudio.play();
            if(p!==undefined){p.then(()=>{isSwitchingMusic=false;}).catch(()=>{isSwitchingMusic=songIsPlayed=false;broadcastGuarantor_elmnt.classList.remove("click");});}
            songIsPlayed=true;broadcastGuarantor_elmnt.classList.add("click");preloadNextSongHead(indexSong+1>songsLength?0:indexSong+1);
        }else{isSwitchingMusic=false;preloadAbortController?.abort();}
        savePlaybackState();
    }
    ...
    function updateTheProgressBar(){
        const d=this.duration,c=this.currentTime;if(isNaN(d)||d===0)return;
        const r=c/d;setProperty(progress_elmnt,"--scale",r);setProperty(progress_elmnt,"--width",`${r*100}%`);
    }
    function syncMediaSessionPosition(){
        if('mediaSession' in navigator&&!isNaN(mainAudio.duration)&&isFinite(mainAudio.duration)&&mainAudio.duration>0){
            try{let p=Math.max(0,Math.min(mainAudio.currentTime||0,mainAudio.duration));navigator.mediaSession.setPositionState({duration:mainAudio.duration,playbackRate:mainAudio.playbackRate||1,position:p});}catch(e){}
        }
    }
    function handleScrub(e){
        e.preventDefault();let cx=e.touches?e.touches[0].clientX:e.clientX;
        const rect=progress_elmnt.getBoundingClientRect(),d=selectedSong.duration;if(isNaN(d)||d===0)return;
    ...
    function controlSubtitleAnimation(p,c){
        if(c.classList.contains("animate"))return;const el=c.firstChild;
...
function initKeyboardControls(){
    document.addEventListener('keydown',e=>{
        ...
        if(key==='t'){currentBgIndex=(currentBgIndex+1)%bgPresets.length;const p=bgPresets[currentBgIndex];switchBackground(p.value,p.isLight);}
        ...
        if(key==='4'){currentBgIndex=currentBgIndex===3?4:3;const p=bgPresets[currentBgIndex];switchBackground(p.value,p.isLight);}
        ...
        if(!e.shiftKey){
            ...
            if((e.key==='ArrowUp'||e.key==='ArrowDown')&&currentPage===1){
                e.preventDefault();const pv=document.querySelector('#page1 .page-vertical');
...
(async function oneko(){
    ...
    const skinList=[{name:"classic",url:"images/oneko-classic.gif"},{name:"black2",url:"images/black2.png"},{name:"black",url:"images/black.png"},{name:"calico",url:"images/calico.png"},{name:"blue",url:"images/blue.png"},{name:"holiday",url:"images/holiday.png"},{name:"kina",url:"images/kina.png"},{name:"lucky",url:"images/lucky.png"},{name:"marmalade",url:"images/marmalade.png"},{name:"mermaid",url:"images/mermaid.png"},{name:"usa",url:"images/usa.png"},{name:"neon",url:"images/neon.png"},{name:"pink",url:"images/pink.png"},{name:"socks",url:"images/socks.png"},{name:"dog",url:"images/dog.png"}];
    ...
    skinList.forEach(s=>{
        ...
        item.style.cssText=`width:32px;height:32px;background-color:rgba(0,0,0,0);background-image:url('${s.url}');background-position:-222px -95px;image-rendering:pixelated;cursor:pointer;border-radius:4px;border:2px solid transparent;transition:border 0.2s, transform 0.2s;`;
        item.title=s.name;item.onmouseenter=()=>{item.style.border="1px solid #c299ff";item.style.transform="scale(1.1)";};item.onmouseleave=()=>{item.style.border="1px solid transparent";item.style.transform="scale(1)";};item.onclick=e=>{e.stopPropagation();changeSkin(s.url);};skinMenu.appendChild(item);
    });
    ...
    function showSkinMenu(){
        skinMenu.style.display="grid";const w=skinMenu.offsetWidth,h=skinMenu.offsetHeight,rect=nekoEl.getBoundingClientRect();
        skinMenu.style.left=rect.left<window.innerWidth/2?`${rect.right+10}px`:`${rect.left-w-10}px`;
        let top=rect.top+(rect.height/2)-(h/2);if(top+h>window.innerHeight)top=window.innerHeight-h-10;
    ...
    window.onekoCycleSkin=()=>{
        ...
        let idx=(skinList.findIndex(s=>s.url===cur)+1)%skinList.length;changeSkin(skinList[idx].url);
    };
    ...
    function create(){
        ...
        const saved=localStorage.getItem("oneko:skin")||"images/oneko-classic.gif";nekoEl.id="oneko";nekoEl.style.width=nekoEl.style.height="32px";nekoEl.style.position="fixed";nekoEl.style.backgroundImage=`url('${saved}')`;
        nekoEl.style.imageRendering="pixelated";nekoEl.style.left=`${nekoPosX-16}px`;nekoEl.style.top=`${nekoPosY-16}px`;nekoEl.style.filter=kuroNeko?"invert(100%)":"none";nekoEl.style.zIndex="9999";nekoEl.style.touchAction=nekoEl.style.userSelect=nekoEl.style.webkitUserSelect="none";document.body.appendChild(nekoEl);
        function updateMousePos(e){
            ...
            let offL=window.visualViewport?.offsetLeft||0,offT=window.visualViewport?.offsetTop||0;mousePosX=e.clientX+offL;mousePosY=e.clientY+offT;
        }
        window.addEventListener("mousemove",e=>!forceSleep&&updateMousePos(e));window.addEventListener("pointermove",e=>!forceSleep&&e.pointerType==="touch"&&updateMousePos(e));
        ...
        nekoEl.addEventListener("pointerdown",e=>{
            if(e.button!==0&&e.pointerType==="mouse")return;e.preventDefault();grabbing=true;let isDragging=false;try{nekoEl.setPointerCapture?.(e.pointerId);}catch(_){}
            let startX=e.clientX,startY=e.clientY,startNekoX=nekoPosX,startNekoY=nekoPosY,grabInterval;
            const moveHandler=e=>{
                e.preventDefault();const dx=e.clientX-startX,dy=e.clientY-startY,adx=Math.abs(dx),ady=Math.abs(dy);
                ...
                if(grabStop||adx>10||ady>10||Math.sqrt(dx**2+dy**2)>10){grabStop=false;clearTimeout(grabInterval);grabInterval=setTimeout(()=>{grabStop=true;nudge=false;startX=e.clientX;startY=e.clientY;startNekoX=nekoPosX;startNekoY=nekoPosY;},150);}
                nekoPosX=startNekoX+dx;nekoPosY=startNekoY+dy;nekoEl.style.left=`${nekoPosX-16}px`;nekoEl.style.top=`${nekoPosY-16}px`;
            };
            const upHandler=e=>{
    ...
    function idle(){
        ...
        if(idleTime>10&&Math.floor(Math.random()*25)===0&&idleAnimation==null){
            ...
            if(nekoPosX<offL+32)scW=true;if(nekoPosY<offT+32)scN=true;if(nekoPosX>offL+vW-32)scE=true;if(nekoPosY>offT+vH-32)scS=true;
            ...
            let walls=[];if(scW)walls.push("scratchWallW");if(scN)walls.push("scratchWallN");if(scE)walls.push("scratchWallE");if(scS)walls.push("scratchWallS");
    ...
    function frame(){
        ...
        if(forceSleep&&Math.abs(dy)<nekoSpeed&&Math.abs(dx)<nekoSpeed){nekoPosX=mousePosX;nekoPosY=mousePosY;nekoEl.style.left=`${nekoPosX-16}px`;nekoEl.style.top=`${nekoPosY-16}px`;idle();return;}
        if((dist<nekoSpeed||dist<48)&&!forceSleep){idle();return;}
        idleAnimation=null;idleAnimationFrame=0;if(idleTime>1){setSprite("alert",0);idleTime=Math.min(idleTime,7)-1;return;}
        let dir=dy/dist>0.5?"N":"";dir+=dy/dist<-0.5?"S":"";dir+=dx/dist>0.5?"W":"";dir+=dx/dist<-0.5?"E":"";if(!dir)dir="idle";setSprite(dir,frameCount);
        nekoPosX-=(dx/dist)*nekoSpeed;nekoPosY-=(dy/dist)*nekoSpeed;
        let offL=window.visualViewport?.offsetLeft||0,offT=window.visualViewport?.offsetTop||0;
        let vW=window.visualViewport?.width||window.innerWidth,vH=window.visualViewport?.height||window.innerHeight;
        nekoPosX=Math.min(Math.max(offL+16,nekoPosX),offL+vW-16);nekoPosY=Math.min(Math.max(offT+16,nekoPosY),offT+vH-16);nekoEl.style.left=`${nekoPosX-16}px`;nekoEl.style.top=`${nekoPosY-16}px`;
    }
    create();
})();
...
class CyberCat {
  ...
  meow() {
    console.log("eep! get away!! ฅ'ω'ฅ");
  }
  ...
  #calculatePositions() {
    ...
    const card = document.querySelector(".glowing-card");
    ...
    const positions = [
      ...
      {
        x: 0,
        y: Math.floor(Math.random() * Math.max(10, winH - this.#width)),
        isLeft: true
      },
      // 1: 屏幕最右侧（使用右边探头图片）
      {
  ...
  teleportRandom() {
    ...
    if (candidates.length > 1) {
      ...
      } while (nextIndex === this.#currentLocationIndex);
    } else {
      nextIndex = 0;
    }
  ...
  mouseCheck(mouseX, mouseY) {
    // 检测鼠标是否进入猫咪的敏感范围
    const inX = mouseX >= (this.#posX - this.#sensitivity) && 
                mouseX <= (this.#posX + this.#width + this.#sensitivity);
    const inY = mouseY >= (this.#posY - this.#sensitivity) && 
                mouseY <= (this.#posY + this.#width + this.#sensitivity);
...
function spawnCyberCat() {
  cyberCat = new CyberCat(0, 50); // 灵敏度为 50px
}
